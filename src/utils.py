...
class ParameterSearch:
    ...
    def __init__(self,
        ...
        self.searchable_params = {**searchable_index_params, **searchable_shared_params, **searchable_eval_params}
    ...
    def create_configs(self, runcount):
        ...
        if self.search_method == 'grid_search':
            ...
            for key, values in self.searchable_params.items():
                if len(values) > 1: # only variations to try when more than one option
                    for option in values:
                        # for the first step we need to try all options, for subsequent steps we need not repeat the default starting options (pipeline_config)
                        if step == 0 or not (key in pipeline_config and option == pipeline_config[key]):
            ...
            if len(self.results):
                pipeline_config = pd.DataFrame(self.results).sort_values(self.target_metric, ascending=False).iloc[0].to_dict()
                pipeline_config.update({'embed_model' : (pipeline_config['embed_model_provider'], pipeline_config['embed_model'])}) # TOD0 cleanify
