...
def read_latest_data(base_path: str, pipeline_name: str, component_name: str):
    ...
    valid_entries = [entry for entry in subdirectories if entry.startswith(pipeline_name)]
    # keep pipeline folders containing a parquet file in the component folder
    valid_entries = [folder for folder in valid_entries if has_parquet_file(data_directory, folder, component_name)]
    ...
    if latest_folder:
        ...
        if parquet_files:
            ...
            print("No Parquet files found in the component folder.")
    else:
        print("No valid folders found with a component folder.")
